...
export default function Benchmarks() {
  ...
  return (
    <>
      <PageHeader title={t("Performance Benchmarks")} subtitle={t("Deployment metrics, latency percentiles, and system resources")}>
        <div className="btn-group">
          {connected && <span className="live-dot" title={t("Live")} />}
          <button className="btn btn-secondary" onClick={load}>{t("Refresh")}</button>
        </div>
      </PageHeader>

      <div className="section">
        <div className="section-title">{t("Deployment Performance")}</div>
      ...
      <div className="section">
        <div className="section-title">{t("API Latency (Deploy Endpoint)")}</div>
        <div className="stats-grid">
          <StatCard label={t("Avg Latency")} value={`${lat.avg ?? 0}ms`} />
      ...
      <div className="section">
        <div className="section-title">{t("Simulation Throughput")}</div>
        <div className="stats-grid">
          ...
          <StatCard label={t("Current EPS Target")} value={live?.current_eps_target ?? 0} meta={t("Active simulations")} />
        </div>
      </div>

      <div className="section">
        <div className="section-title">{t("Live System Resources")}</div>
        <div className="stats-grid">
          <StatCard label={t("Memory Used")} value={`${sys.memory_used_percent ?? 0}%`} color={sys.memory_used_percent > 85 ? "red" : sys.memory_used_percent > 70 ? "orange" : "green"} meta={`${sys.memory_available_mb ?? 0} MB free`} />
          <StatCard label={t("Disk Used")} value={`${sys.disk_used_percent ?? 0}%`} color={sys.disk_used_percent > 90 ? "red" : undefined} />
          <StatCard label={t("CPU Load (1m)")} value={sys.load_average?.[0]?.toFixed(2) ?? "—"} meta={`${sys.cpu_count ?? "?"} cores`} />
          <StatCard label={t("Containers Running")} value={live?.by_status?.running ?? 0} color="green" meta={`${live?.total_agents ?? 0} total`} />
        </div>
      </div>

      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16 }}>
        <div className="card">
          <div className="section-title">{t("Memory % (history)")}</div>
          <MiniChart data={sysHistory.memory} label={t("Mem%")} color="var(--orange)" />
        </div>
        <div className="card">
          <div className="section-title">{t("CPU Load 1m (history)")}</div>
          <MiniChart data={sysHistory.cpu} label={t("Load")} color="var(--cyan)" />
        </div>
        <div className="card">
          <div className="section-title">{t("Running Containers (history)")}</div>
          <MiniChart data={sysHistory.containers} label={t("Running")} color="var(--green)" />
        </div>
      </div>

      {benchmarks && (
        <div className="section" style={{ marginTop: 20 }}>
          <div className="section-title">{t("Benchmark data")}</div>
